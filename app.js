...
let isInitialCloudFetchComplete = false; // Prevents "No Entries Found" flash while cloud is loading
...
function canonicalId(id) {
  ...
  return str.startsWith("ej-") ? str : "ej-" + str;
}
...
function deduplicateEntries(entries) {
  ...
  const uniqueList = Array.from(new Set(idMap.values()));
  uniqueList.sort((a, b) => safeParseDate(b.date || b.createdAt || b.updatedAt) - safeParseDate(a.date || a.createdAt || a.updatedAt));
  return uniqueList;
}
...
const DB = {
  ...
  normalizeEntry(entry) {
    ...
    if (!raw && !vic) {
      try {
        ...
        const match = [...publicArr, ...privateArr].find(e => e && canonicalId(e.id) === entry.id && (e.rawContent || e.victorianContent || e.publicContent));
        if (match) {
  ...
  getPublicEntries() {
    ...
    try {
      ...
      return Array.isArray(parsed) ? deduplicateEntries(parsed.map(e => this.normalizeEntry(e))) : [];
    } catch(e) {
      return [];
    }
  },
  ...
  savePrivateEntries(entries) {
    const clean = deduplicateEntries(entries);
    this.safeSetItem("ej_entries_private", JSON.stringify(clean));
    this.createLocalSnapshot(clean);
  },
    ...
    return new Promise((resolve, reject) => {
      ...
      reader.onload = async (e) => {
        try {
          ...
          const merged = deduplicateEntries([...existingPrivate, ...data.entries.map(ent => this.normalizeEntry(ent))]);
  ...
  async saveEntry(entry, rawContent = null, victorianContent = null, preserveUpdatedAt = false) {
    ...
    const privateEntry = {
      ...
      isRawFallback: entry.isRawFallback === true,
      createdAt: entry.createdAt || now,
      updatedAt: updatedAt
    };
  ...
  async fetchPublicCloudEntries(onEntryStream) {
    ...
    try {
      ...
      querySnapshot.forEach(doc => {
        const data = doc.data();
        if (data) {
          data.id = canonicalId(data.id || doc.id);
          cloudEntries.push(this.normalizeEntry(data));
        }
      });
